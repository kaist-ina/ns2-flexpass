...
int TcpXPassAgent::command(int argc, const char*const* argv) {
  if (argc == 2) {
...
Packet* TcpXPassAgent::construct_credit() {
  ...
  if (min_credit_size_ < max_credit_size_) {
    ...
    credit_size += Random::uniform(max_credit_size_ - min_credit_size_ + 1);
  } else {
    // static credit size
    if (min_credit_size_ != max_credit_size_){
